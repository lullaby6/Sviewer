...
document.querySelector(".bx-reset").addEventListener("click", (event) => {
	reset();
});

document.querySelector(".bx-chevron-left").addEventListener("click", (event) => {
	if(pos > 0){
		pos -= 1;
	}else{
...
document.querySelector(".bx-chevron-right").addEventListener("click", (event) => {
	if(pos < upload.files.length-1){
		pos += 1;
	}else{
		pos = 0;
	}
